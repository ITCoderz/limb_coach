...
class ChatScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          children: [
            ClipRRect(
              ...
              child: Image.asset(user.image, height: 49),
            ),
            SizedBox(width: 8),
            Column(
        ...
        actions: [
          GestureDetector(
            child: Image.asset(Assets.pngIconsAudioCall, height: 40, width: 40),
            onTap: () =>
                Get.to(() => CallScreen(name: user.name, image: user.image)),
          ),
          10.pw,
          GestureDetector(
            child: Image.asset(Assets.pngIconsVideoCall, height: 40, width: 40),
            onTap: () =>
                Get.to(() => CallScreen(name: user.name, image: user.image)),
          ),
          20.pw,
  ...
  Widget _buildInputBar() {
    return Obx(() {
      final isTyping = c.isTyping.value;
      ...
      return Container(
        ...
        decoration: BoxDecoration(
          border: Border.all(
            color: isTyping ? AppColors.primaryColor : AppColors.borderColor,
            width: isTyping ? 1 : 0.5,
          ),
        ...
        child: Row(
          children: [
            ...
            GestureDetector(
              child: Image.asset(Assets.pngIconsGetFiles, height: 30),
              onTap: () {
                c.sendImageMessage();
              },
            ),
            ...
            GestureDetector(
              child: isTyping
                  ? Image.asset(Assets.pngIconsSendMessageBlue, height: 30)
                  : Image.asset(Assets.pngIconsSendMessage, height: 30),
              onTap: () {
                if (msgController.text.trim().isNotEmpty) {
                  c.sendMessage(msgController.text.trim());
                  msgController.clear();
                  c.isTyping.value = false;
                }
              },
            ),
          ],
